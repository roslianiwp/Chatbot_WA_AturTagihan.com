...
with open("tests/unit_type/delete.json") as file:
    delete_unit_type = json.load(file)
...
class TestJenisUnit():
    ...
    def mocked_requests_aturtagihan_delete_id_jenis_unit(*args, **kwargs):
        class MockResponse:
            def __init__(self, json_data, status_code):
                self.json_data = json_data
                self.status_code = status_code
        ...
        if len(args) > 0:
            if args[0] == "https://magellan.sumpahpalapa.com/api/v1/unit_type/775":
                return MockResponse(delete_unit_type, 400)
    ...
    @mock.patch('requests.delete', side_effect=mocked_requests_aturtagihan_delete_id_jenis_unit)
    def test_delete_jenis_unit_success(self, delete_mock, post_mock, client):
        # PENGATURAN
    ...
    # HAPUS 100
    @mock.patch('requests.post', side_effect=mocked_requests_aturtagihan_get_all_jenis_unit)
    def test_delete_jenis_unit_100(self, post_mock, client):
        # PENGATURAN
        data = send_message("6281320003997", "pengaturan")
        res = client.post('/webhooks/inbound-message',
                            data=json.dumps(data),
