...
class TestUnit():
    ...
    def mocked_requests_aturtagihan_delete(*args, **kwargs):
        class MockResponse:
            ...
            def json(self):
                return self.json_data
        if len(args) > 0:
            if args[0] == "https://magellan.sumpahpalapa.com/api/v1/unit/3204":
                return MockResponse(add_unit, 200)
            else:
                return MockResponse(None, 404)
    ...
    def test_delete_unit_success(self, delete_mock, post_mock, client):
        # PENGATURAN
    ...
    @mock.patch('requests.post', side_effect=mocked_requests_aturtagihan_get_all)
    @mock.patch('requests.delete', side_effect=mocked_requests_aturtagihan_delete)
    def test_delete_unit_100(self, delete_mock, post_mock, client):
        # PENGATURAN
        data = send_message("6281320003997", "pengaturan")
        res = client.post('/webhooks/inbound-message',
                            data=json.dumps(data),
